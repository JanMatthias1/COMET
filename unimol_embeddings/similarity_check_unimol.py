...
def main():
    parser = argparse.ArgumentParser(
        description='Cosine similarity matrix for Unimol embeddings'
    )
    ...
    if args.names_csv:
        ...
        # --- diagnostic ---
        matched = [s for s in smiles_list if s in smiles_to_name]
        unmatched = [s for s in smiles_list if s not in smiles_to_name]
        print(f"Name matches: {len(matched)}/{n}")
        if unmatched:
            print("Unmatched SMILES (falling back to truncated string):")
            for s in unmatched:
                print(f"  {s}")
    ...
    ax.set_title('Pairwise Cosine Similarity of Unimol Lipid Embeddings',
                 fontsize=13, pad=20)
    plt.tight_layout()
